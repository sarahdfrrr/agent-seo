...
description: |
  ...
  Trigger: "humanize this", "humanise ce texte", "ça sonne trop IA", "rends ça plus naturel",
  "relis l'article et enlève les tics IA", "/humanizer". Run this after seo-writer produces
  an article, and before publishing any customer-facing copy (emails, social posts, job outreach).
license: MIT
metadata:
  version: "3.0.0"
...
Vendored from [blader/humanizer](https://github.com/blader/humanizer) at `v3.0.0` (commit `9862685`). To update, re-sync `SKILL.md` from that repo and bump this version.

## Related Skills

- **seo-writer**: writes the article; run humanizer on the final draft before publishing.
- **seo-strategy**: unaffected by this skill; it plans content, it does not write prose.
